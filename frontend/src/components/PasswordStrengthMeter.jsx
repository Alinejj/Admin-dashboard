...
const PasswordCriteria = ({password}) => {
    const criteria =[
        ...
        {label: "Contains at least one special character", met: /[^A-Za-z0-9]/.test(password)},
    ];
        ...
        <div className='mt-2 space-y-1'>
            {criteria.map((item) =>(
                <div key={item.label} className='flex items-center text-xs'>
                    ...
                    <span className={item.met ? 'text-pink-400' : 'text-gray-500'}> 
                        {item.label}
                    </span>
...
const PasswordStrengthMeter = ({password}) => {
  ...
  return <div className='mt-2'>
    <div className='flex justify-between items-center mb-1'>
        ...
        <span className='text-xs test-gray-400'> {getStrengthText(strength)}</span>
    </div>
